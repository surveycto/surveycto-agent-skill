...
An [Agent Skill](https://agentskills.io) that gives AI agents SurveyCTO domain expertise: designing, editing, and debugging [SurveyCTO](https://www.surveycto.com) forms (XLSForm `.xlsx` files), server datasets (XML definitions), Data Explorer workbook definitions, and field plug-ins (`.fieldplugin.zip` bundles). It can also **translate** open-ended response columns in an exported CSV and **transcribe** audio captures (audio audits, voice responses) via OpenAI; these workflows need a user-provided OpenAI API key and outbound network access to `api.openai.com` (see [How to use](#how-to-use) and the credential primer).
...
* We highly encourage you to review your organization's compliance, data security, and AI usage policies prior to using this skill. In particular, the optional translate/transcribe workflows send the selected response text or audio to **OpenAI** (a third-party service); the skill always shows a cost estimate and a privacy warning and requires explicit confirmation before any such call, but confirm this transfer is acceptable under your data-governance rules first.
...
In principle, the skill activates automatically when you:
...
- Ask to **translate** exported response/comment columns or **transcribe** audio captures (these run via OpenAI and need an API key + `api.openai.com` egress; the skill coaches the setup)

**In practice, automatic activation is unreliable** — agents often answer SurveyCTO questions from their generic training data without ever loading the skill, which defeats the point. To guarantee the skill is used, invoke it explicitly:

- In hosts that support slash-command skill invocation (e.g., Claude Cowork), start your message with `/surveycto`.
- In any host, prefix your request with something like "Using the SurveyCTO skill, …" or "Load the SurveyCTO skill and then …".

After sending the request, check the agent's response (or its tool-use trace) to confirm it actually loaded the skill — most hosts show a "Skill: surveycto" badge, a `skill` tool call, or a similar indicator. If you don't see one, the agent is winging it, and the answer is likely to disappoint. Re-prompt explicitly until the skill is loaded.
...
| `references/openai-credentials.md` | OpenAI key onboarding (file handoff, never via chat), network-egress guidance, spend tracking |
| `references/user-data-translation.md` | Translating exported response/comment columns via OpenAI |
| `references/audio-transcription.md` | Transcribing audio captures via OpenAI (chunking, cost gate, PII warning) |
| `assets/transcribe-translate/` | Runtime scripts for the OpenAI translate/transcribe workflows (`setup_env`, `openai_auth`, `translation`, `transcription`, `usage_ledger`, `pricing`) |
