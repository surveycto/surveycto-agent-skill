...
| Referencing inside a repeat | From within the same repeat instance, `${field}` refers to that instance's field. For the current repeat instance number, put `index()` in a `calculate` and reference that field; `${index()}` written directly in a label renders nothing, because `${...}` resolves a field name, not a function. |
