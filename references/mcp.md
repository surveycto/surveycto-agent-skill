...
- **Parse findings:** Every response carries two top-level keys, `parse_errors` (broken `${name}` dependencies, mismatched begin/end group boundaries, duplicate field names) and `parse_warnings` (softer parser advisories). They are the re-parse of the current workbook state and are distinct from `summary.warnings`, which carries patch-application warnings only (such as `unknown_column_added`). `start_xlsform_session`, `get_xlsform_summary`, and `xls_apply_patches` all return these same keys, so you read findings from one place. They describe the current parse state of the whole workbook, not just the deltas from one batch, so a pre-existing break persists across later batches until the offending rows are fixed. After any `delete_row` or `rename`, check `parse_errors` and fix or roll back dangling references in the same batch.
- **Large batches:** Set `return_form_summary=false` and `include_details=false`, then call `get_xlsform_summary` only when you need a fresh summary. `parse_errors` / `parse_warnings` are returned regardless of `return_form_summary`, so the orphan-reference signal survives compact responses.
- **Dry runs:** Use `validate_only=true` for risky changes. The dry run re-parses the patched bytes in memory and populates `parse_errors` / `parse_warnings` without bumping the version or writing anything, so you can pre-flight a structural edit (for example a `delete_row` that would orphan a `${name}` reference) before committing. If the in-memory re-parse fails, the dry run does not fail; it reports a `parse_check_inconclusive` entry in `parse_warnings`, so treat an empty `parse_errors` as clean only when that entry is absent.
...
- **One exception, after a structural edit:** a `delete_row`, a `rename`, or a row-reorder workaround can leave `${name}` references dangling. After those, glance at `parse_errors` on the same response (it is always there, even with `return_form_summary=false`). A non-empty list means the batch left broken references, mismatched group boundaries, or duplicate names that the per-op counts alone would not reveal. This is a cheap field read, not a follow-up call, so it does not reintroduce the anxiety loop.
